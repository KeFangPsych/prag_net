...
from typing import List, Dict, Union, Optional, Tuple, TypeVar, Iterator, Callable, Any
...
def log_M_product(LA, LB, precise=True, margin=None):
    """
...
def log_column_normalize(LX, precise=True):
    """
    ...
    precise (bool): If True (default), uses a highly precise custom implementation
    ...
    # Set floating-point error reporting for over- and underflow to warn.
    np.seterr(over='warn', under='warn')
    ...
    if not precise:
        ...
        M_LX = np.max(LX, axis=0)  # shape: (1, n)
...
def log_column_softmax(X, alpha, precise=True):
    """
    ...
    # Scoped floating-point warnings
...
def spacious_logsumexp(x, axis=None, keepdims=False, precise=True, margin=None):
    """
    Compute log-sum-exp with optional high-precision mode.

    By default, this is a wrapper around scipy.special.logsumexp.
    When precise=True, uses an advanced algorithm that
      prevent overflow and reduce chance of underflow in exponentiation
      and high-precision computation in summation.
    """
    ...
    if not precise:
        # Standard mode: delegate to scipy
        from scipy.special import logsumexp
        return logsumexp(x, axis=axis, keepdims=keepdims)

    else:
        # 1) figure out which axes to reduce
        if axis is None:
            axes = tuple(range(x.ndim))
        else:
            axes = normalize_axis_tuple(axis, x.ndim)

        # 2) compute the “headroom” shift constant
        M_max = np.finfo(x.dtype).max
        if margin is None:
            margin = -math.log(np.finfo(x.dtype).eps)
        # vectorized max over reduction axes, keep the dims so we can broadcast
        max_x = np.max(x, axis=axes, keepdims=True)
        max_neginf = max_x == -np.inf

        # total elements per slice & log(n)
        slice_size = int(np.prod([x.shape[a] for a in axes], dtype=int))
        log_n = math.log(slice_size)

        # build the shift m as a small array (one entry per slice, broadcastable)
        m = max_x + log_n - math.log(M_max) + margin
        m = np.where(max_neginf, 0, m)

        # 3) subtract shift (broadcast), then move reduction axes to the end
        shifted = x - m
        # move the axes we’re going to sum over into the last positions
        shifted = np.moveaxis(shifted, axes, range(-len(axes), 0))
        # now flatten into rows of length slice_size
        rows = shifted.reshape(-1, slice_size)

        # 4) do the only-in-Python part: exact fsum of each row’s exponentials
        sum_exps_shifted = np.array([math.fsum(np.exp(r)) for r in rows], dtype=x.dtype)

        # 5) pull out the matching shifts for each row
        m_flat = np.moveaxis(m, axes, range(-len(axes), 0)).reshape(-1)

        # 6) finish up: m + log(fsum) → flat output, then reshape & keepdims
        sum_exps_shifted_zeros = sum_exps_shifted == 0.0
        log_sum_exps_shifted = np.log(np.clip(sum_exps_shifted, np.finfo(float).tiny, None))
        log_sum_exps_shifted = np.where(sum_exps_shifted_zeros, -np.inf, log_sum_exps_shifted)
        out = m_flat + log_sum_exps_shifted
        # the output’s “non-reduced” shape is simply x.shape with those axes removed
        non_red_shape = [x.shape[i] for i in range(x.ndim) if i not in axes]
        out = out.reshape(non_red_shape)
...
class LiteralSpeaker:
    ...
    def current_belief_theta(self) -> np.ndarray:
        """
        Return the current log-beliefs normalized so they form a valid distribution.

        Exponentiating and summing to 1.
        """
        return np.exp(log_column_normalize(self.un_current_log_belief[:, None],
                                           precise= precision).ravel())
    ...
    def current_belief_theta(self) -> np.ndarray:
        ...
        normalized = np.exp(
            log_column_normalize(log_bel[:, None],
                                 precise= precision)  # shape: (theta,1)
        ).ravel()  # back to shape (theta,)
        return normalized



## level-n Pragmatic Listener
...
## level-2 and plus Pragmatic Speaker

class PragmaticSpeaker_obs_2plus:
    """
    A level-n (n>1) pragmatic speaker (S_n) in an RSA-style communication game.

    This speaker balances literal truth, informativeness, and persuasiveness
    when choosing an utterance, with an internal pragmatic listener (L_(n-1)).
    """
    VALID_OMEGA_TYPES = {"coop", "strat"}
    VALID_PSI_TYPES = {"inf", "pers+", "pers-"}
    ...
    def __init__(
        ...
        if psi not in self.VALID_PSI_TYPES:
            raise ValueError(f"psi must be one of {self.VALID_PSI_TYPES}, got '{psi}'")
        ...
        # Enforce constraint from formalization: in coop worlds, speakers must be informative
        if self.omega == "coop" and psi != "inf":
            warnings.warn("when omega == coop, psi must equal to inf ", UserWarning)
            self.psi = "inf"
        else:
            self.psi = psi
        ...
        # Use same alpha values for internal models if not specified
        #if alpha_vals is None:
        #    alpha_vals = [alpha]

        try:
            # Initialize literal speaker for optimal Bayesian belief updating
            self.internal_speaker = LiteralSpeaker(
                world=world,
                initial_beliefs_theta=initial_beliefs_theta
            )

            # Initialize pragmatic listener as internal listener model
            self.pragmatic_listener = PragmaticListener_obs_n(
                world=world,
                level = self.level - 1,
                omega=omega,
            ...
            raise RuntimeError(f"Failed to initialize level-2 pragmatic speaker: {str(e)}")


    def _compute_log_informativeness(
        self,
        obs_log_likelihood_theta_values: np.ndarray,
        un_current_log_belief_theta_psi_alpha_joint_values: np.ndarray,
        internal_speakers_of_listener: Dict[Tuple[str, Union[float, str]], 'PragmaticSpeaker_obs']
        ) -> Tuple[np.ndarray, pd.DataFrame]:
        """
        Compute log-Inf(u; O) = log-P_L1|ω(O|u).
        # self.world.obs_log_likelihood_theta.values
        # self.pragmatic_listener.un_current_log_belief_theta_psi_alpha_joint.values
        # self.pragmatic_listener.pragmatic_speakers[(psi, a)].utterance_log_prob_obs.values
        ...
        obs_log_likelihood_theta_values : np.ndarray
            Observation log-likelihoods for each theta.
        un_current_log_belief_theta_psi_alpha_joint_values : np.ndarray
            Unnormalized log-beliefs over theta, psi, alpha values.
        internal_speakers_of_listener : Dict[Tuple[str, Union[float, str]], 'PragmaticSpeaker']
            interal speaker of the internal listener, used to calculate
            Log P(u|O, psi, alpha) for all utterances and observations.
        ...
        """
        try:
            # 1. Compute log-P_{L_1}(O, psi, alpha)

            theta_vals = self.world.theta_values
            psi_vals = self.pragmatic_listener.psi_vals
            alpha_vals = self.pragmatic_listener.alpha_vals
            obs_vals = self.world.possible_outcomes
            ...
            # Get the current joint belief
            log_joint = un_current_log_belief_theta_psi_alpha_joint_values
            # self.pragmatic_listener.un_current_log_belief_theta_psi_alpha_joint.values

            # Reshape log_joint to [n_theta, n_psi*n_alpha]
            reshaped_log_joint = log_joint.reshape(n_theta, -1)

            # Multiply and marginalize over theta
            ...
            log_prob_O_psi_alpha = xr.DataArray(
                data=result.reshape(n_O, n_psi, n_alpha),
                dims=("observation", "psi", "alpha"),
                coords={
                    "observation": pd.Index(self.world.possible_outcomes, dtype=object, tupleize_cols=False),
                    "psi": psi_vals,
                    "alpha": alpha_vals
                },
                name="log P_L1(O, psi, alpha)"
            )

            # Allocate buffer: (psi, alpha, u, theta)
            log_P_u_given_O_psi_alpha_buf = np.empty((n_psi, n_alpha,
                len(self.world.utterances), len(self.world.possible_outcomes)), dtype=float)

            # Loop over each speaker configuration
            for i, psi in enumerate(psi_vals):
              for j, a in enumerate(alpha_vals):
                                ps = internal_speakers_of_listener[(psi, a)]
                                # log P_S1(u | O) matrix
                                log_P_u_given_O_psi_alpha_buf[i, j, :, :] = ps.utterance_log_prob_obs.values

            log_P_u_given_O_psi_alpha = xr.DataArray(
                data=log_P_u_given_O_psi_alpha_buf,
                dims=("psi", "alpha", "utterance", "observation"),
                coords={
                    "psi": psi_vals,
                    "alpha": alpha_vals,
                    "utterance": self.world.utterances,
                    "observation": pd.Index(self.world.possible_outcomes, dtype=object, tupleize_cols=False)
                    },
                name="log P_S1(u | observation, psi, alpha)").transpose("utterance", "observation", "psi", "alpha")
            ...
            log_P_O_given_u = un_log_P_O_given_u - logsumexp(un_log_P_O_given_u, axis = (1), keepdims = True)

            log_informativeness = pd.DataFrame(
                log_P_O_given_u,
                index=self.world.utterances,
                columns=self.world.possible_outcomes
            )
            ...
            return log_prob_O_psi_alpha, log_informativeness

        except Exception as e:
            raise RuntimeError(f"Failed to compute informativeness: {str(e)}")

    def _compute_log_persuasiveness(
        self,
        psi: str,
        theta_values: np.ndarray,
        theta_psi_alpha_log_post_utterance_values: np.ndarray
    ) -> Tuple[pd.DataFrame, pd.DataFrame]:
        """
        Compute log-PersStr(u; psi) based on speaker type.
        ...
        theta_values : np.ndarray
            Array of possible theta values.
        theta_psi_alpha_log_post_utterance_values : np.ndarray
            Unnormalized log-posteriors for each theta, psi, alpha given utterance.
        ...
        try:
            ...
            theta_values_zero = theta_values == 0.0
            log_theta_values = np.log(np.clip(theta_values, np.finfo(float).tiny, None))
            log_theta_values = np.where(theta_values_zero, -np.inf, log_theta_values)
            ...
            raise RuntimeError(f"Failed to compute persuasiveness: {str(e)}")
    ...
    def _compute_utility(self, psi: str, beta: float) -> pd.DataFrame:
        ...
        try:
            # Compute informativeness
            self.unnormalized_log_prob_O, self.log_informativeness = self._compute_log_informativeness(
                self.world.obs_log_likelihood_theta.values, # LP(O |theta)
                self.pragmatic_listener.un_current_log_belief_theta_psi_alpha_joint.values, # LP(theta, psi, alpha)
                self.pragmatic_listener.pragmatic_speakers) # for each LP(u | O, theta, psi)

            # Compute persuasiveness
            self.theta_log_expectation_utterance, self.log_persuasiveness = self._compute_log_persuasiveness(
                psi = psi,
                theta_values = self.world.theta_values,
                theta_psi_alpha_log_post_utterance_values = self.pragmatic_listener.theta_psi_alpha_log_post_utterance)

            # mask of literally false (Truth=0)
            uttr_false = (self.world.utterance_truth == 0)

            # Implement utility formula from formalization
            if self.psi == "inf":
                # Purely informative (β = 1)
                util = self.log_informativeness.copy()
            elif self.beta == 0:
                # Purely persuasive (β = 0)
                util = self.log_persuasiveness.copy()
            else:
                # Mixed informative & persuasive
                # Mask out any utterance that was impossible in either term
            ...
            # Finally enforce literal truth - this implements Truth(u;O) term
            util[uttr_false] = -np.inf
    ...
    def update_and_speak(self, observation: Tuple[int, ...]) -> str:
        """Given an observation, update beliefs and sample an utterance."""
        try:
            # Bayesian optimal update using observation via literal speaker
            self.internal_speaker.update_and_speak(observation)
            ...
            # Only update internal state if update_internal is True
            if self.update_internal:
                self.pragmatic_listener.listen_and_update(selected_utterance)

                # Recompute utterance probabilities
                self.utterance_log_prob_obs = self._compute_utterance_log_prob_obs(self.alpha)
    ...
    def current_belief_theta(self) -> np.ndarray:
        ...
        (in linear space, summing to 1).
        """
        # Get belief from internal speaker
        return self.internal_speaker.current_belief_theta
...
def create_speaker(
    ...
    ----------
    world : World
        The communication world/scenario
    level : int
        Reasoning level (0 = literal, 1 = pragmatic, 2+ = higher-order pragmatic)
    omega : str, required for level ≥ 1
        World type: "coop" (cooperative) or "strat" (strategic)
    psi : str, required for level ≥ 1  
        Speaker goal: "inf" (informative), "pers+" (persuade up), "pers-" (persuade down)
    update_internal : bool, required for level ≥ 1
        Whether to update internal listener model after speaking
    alpha : float or str, required for level ≥ 1
        Rationality parameter (softmax temperature) or "determ" for deterministic
    beta : float, default=0.0
        Weight on informativeness vs persuasiveness (1.0 = pure info, 0.0 = pure persuasion)
    initial_beliefs_theta : np.ndarray, optional
        Prior beliefs over theta values
    initial_beliefs_psi : np.ndarray, optional  
        Prior beliefs over speaker goal types (for level ≥ 2)
    alpha_vals : List[Union[float, str]], optional
        Grid of possible alpha values for internal models (for level ≥ 2)
    initial_beliefs_alpha : np.ndarray, optional
        Prior beliefs over alpha values (for level ≥ 2)
    
    Returns
    -------
    Speaker instance
        LiteralSpeaker (level 0), PragmaticSpeaker_obs (level 1), 
        or PragmaticSpeaker_obs_2plus (level ≥ 2)
        
    Raises
    ------
    ValueError
        If level is invalid or required parameters are missing
    TypeError
        If world is not a World instance
    """
    ...
    # Note: We can't easily validate World type without circular imports,
    # so we'll let the underlying classes handle that validation
        
    # Create appropriate speaker based on level
    if level == 0:
        ...
        if omega is None:
            raise ValueError("omega parameter is required for level 1 speakers. Must be 'coop' or 'strat'.")
        if psi is None:
            raise ValueError("psi parameter is required for level 1 speakers. Must be 'inf', 'pers+', or 'pers-'.")
        if update_internal is None:
        ...
        if omega is None:
            raise ValueError(f"omega parameter is required for level {level} speakers. Must be 'coop' or 'strat'.")
        if psi is None:
            raise ValueError(f"psi parameter is required for level {level} speakers. Must be 'inf', 'pers+', or 'pers-'.")
        if update_internal is None:
...
def create_listener(
    world: 'World',
    level: int,
    # Required parameters for pragmatic listeners (level ≥ 1)
    omega: Optional[str] = None,
    update_internal: Optional[bool] = None,
    alpha: Optional[Union[float, str]] = None,
    # Optional parameters (have meaningful defaults)
    beta: float = 0.0,
    initial_beliefs_theta: Optional[np.ndarray] = None,
    # Parameters for pragmatic listeners (level ≥ 1)
    initial_beliefs_psi: Optional[np.ndarray] = None,
    alpha_vals: Optional[List[Union[float, str]]] = None,
    initial_beliefs_alpha: Optional[np.ndarray] = None):
    """
    Create RSA listener of specified level with explicit parameter validation.
    ...
    world : World
        The communication world/scenario
    level : int
        Reasoning level (0 = literal, 1+ = pragmatic with joint inference)
    omega : str, required for level ≥ 1
        World type: "coop" (cooperative) or "strat" (strategic)
    update_internal : bool, required for level ≥ 1
        Whether speaker models update their internal state
    alpha : float or str, required for level ≥ 1
        Default rationality parameter for speaker models
    beta : float, default=0.0
        Default weight on informativeness vs persuasiveness for speaker models
    initial_beliefs_theta : np.ndarray, optional
        Prior beliefs over theta values
    initial_beliefs_psi : np.ndarray, optional
        Prior beliefs over speaker goal types (for level ≥ 1)
    alpha_vals : List[Union[float, str]], optional
        Grid of possible alpha values for speaker models (for level ≥ 1)
    initial_beliefs_alpha : np.ndarray, optional
        Prior beliefs over alpha values (for level ≥ 1)
    
    Returns
    -------
    Listener instance
        LiteralListener (level 0) or PragmaticListener_obs_n (level ≥ 1)
    ...
    # Create appropriate listener based on level
    if level == 0:
        # Literal listener - only needs world and optional initial beliefs
        ...
        # Pragmatic listener - validate required parameters
        if omega is None:
            raise ValueError(f"omega parameter is required for level {level} listeners. Must be 'coop' or 'strat'.")
        if update_internal is None:
            raise ValueError(f"update_internal parameter is required for level {level} listeners. Must be True or False.")
        if alpha is None:
            raise ValueError(f"alpha parameter is required for level {level} listeners. Must be a float or 'determ'.")
            
        return PragmaticListener_obs_n(
            world=world,
            level=level,
            omega=omega,
            update_internal=update_internal,
