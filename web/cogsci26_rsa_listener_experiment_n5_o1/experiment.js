...
const PROLIFIC_COMPLETION_CODE = "REPLACE_WITH_YOUR_PROLIFIC_COMPLETION_CODE";
const PROLIFIC_SCREENING_CODE = "REPLACE_WITH_YOUR_PROLIFIC_SCREENING_CODE";
...
// ============================================================================
// EXPERIMENTAL CELL MAP
// ============================================================================
// 24 between-subjects cells = 3 belief × 3 utterance × {1-3} sequence variants.
// CELL_MAP[i] gives the (listener belief, speaker utterance, sequence) for cell
// i. Assignment is uniform via DataPipe modulo CELL_MAP.length below; replace
// the modulo with a custom weighting if you need to balance N per cell against
// existing data.
const CELL_MAP = [
...
// DataPipe returns an integer (configured on the DataPipe dashboard); we map
// it modulo CELL_MAP.length onto one of the 24 between-subjects cells. See
// README.md for how to re-weight if you need to balance against existing data.
...
const assignConditions = {
  ...
  func: function () {
    ...
    // Use DataPipe condition assignment with uniform mapping into the cell grid.
    // (Original study used a weighted map keyed to in-progress N-per-cell targets;
    //  for a fresh public release we just use uniform modulo. See README.md for
    //  how to re-weight if you need to balance against existing data.)
    if (
      experimentState.datapipeCondition !== null &&
      experimentState.datapipeCondition >= 0
    ) {
      experimentState.cellIdx =
        experimentState.datapipeCondition % CELL_MAP.length;
