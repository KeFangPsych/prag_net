...
function showInactivityWarning(message, isUrgent = false) {
  ...
  warningBox.innerHTML = `
        <h2 style="color: ${
          isUrgent ? "#f44336" : "#ff9800"
        }; margin-bottom: 15px;">
            ⚠️ ${isUrgent ? "Urgent: " : ""}Listener Waiting!
        </h2>
        <p style="font-size: 1.1em; margin-bottom: 20px;">${message}</p>
        <button id="dismiss-warning-btn" class="jspsych-btn" style="background: ${
          isUrgent ? "#f44336" : "#ff9800"
...
function startInactivityTimer() {
  ...
  experimentState.inactivityTimer = setInterval(() => {
    ...
    if (elapsed >= CONFIG.inactivity_timeout) {
      ...
      jsPsych.data.addProperties({
        terminated_due_to_inactivity: true,
        inactivity_timeout_at: new Date().toISOString(),
      });
      ...
      jsPsych.endExperiment(`
                <div class="debrief-container">
                    ...
                    <p style="margin-top: 20px; color: #666;">If you believe this was an error, please contact the research team.</p>
                </div>
            `);
    }
  }, 1000);
...
const comp2_trial_1 = {
  ...
  stimulus: function () {
    ...
    return `<div class="comprehension-container">
            <p class="progress-indicator">Question 1 of ${
              CONFIG.comprehension.module2.length
            }</p>
            ...
            <p style="text-align: center; color: #666; font-size: 0.9em;">(${
              item.numEffective
            } effective, ${5 - item.numEffective} ineffective)</p>
            <div class="definition-box" style="text-align: center; font-size: 1.2em;">"${
              item.statement
            }"</div>
...
const comp2_feedback_1 = {
  ...
  stimulus: function () {
    ...
    if (item.statement.includes("Ineffective for Some")) {
      explanation = `"Some" means at least 1. There ${
        numIneffective === 1 ? "is" : "are"
      } ${numIneffective} ineffective patient${
        numIneffective === 1 ? "" : "s"
      }, so this is ${numIneffective >= 1 ? "TRUE" : "FALSE"}.`;
    } else if (item.statement.includes("Ineffective for All")) {
      explanation = `"All" means all 5 patients. There ${
        numIneffective === 1 ? "is" : "are"
      } only ${numIneffective} ineffective patient${
        numIneffective === 1 ? "" : "s"
      }, not all 5, so this is ${numIneffective === 5 ? "TRUE" : "FALSE"}.`;
    }

    if (data.comp2_correct) {
      return `<div class="comprehension-container">
                <h2 style="color: #4CAF50;">✓ Correct!</h2>
                <p>The statement "${item.statement}" is <strong>${
      ...
      return `<div class="comprehension-container">
                <h2 style="color: #f44336;">✗ Incorrect</h2>
                <p>The statement "${item.statement}" is actually <strong>${
        item.correct ? "TRUE" : "FALSE"
      }</strong>.</p>
                <p>${explanation}</p>
...
const comp2_trial_2 = {
  ...
  stimulus: function () {
    ...
    const imgPath = Stimuli.getImagePath(item.numEffective);
    return `<div class="comprehension-container">
            <p class="progress-indicator">Question 2 of ${
              CONFIG.comprehension.module2.length
            }</p>
            <h3>Is this statement TRUE or FALSE?</h3>
            <div class="stimulus-container">
      ...
      html += `<div class="checkbox-option" style="border-color: ${borderColor}; background: ${bgColor};">
        ...
        isCorrect ? "✓" : "✗"
                ...
                <p style="font-size: 0.85em; margin: 5px 0 0 0; color: #666;">
                    ${numIneffective} ineffective ${
        numIneffective > 2.5 ? "> 2.5 ✓" : "≤ 2.5 ✗"
      }
                </p>
            </div>`;
    });
...
const speakerIntro = {
  ...
  stimulus: `<div class="scenario-container">
        ...
        <div class="definition-box" style="margin: 20px 0;">
            <strong>💰 Bonus:</strong> After each description, the listener will respond based on what you told them. 
      ...
      stimulus: function () {
        ...
        return `<div class="trial-container">
                    <div class="trial-header">
                        <span class="round-indicator" style="background:${
                          s.color
                        };">Round ${r + 1} of ${CONFIG.n_rounds} | ${
          s.role
        }</span>
                    </div>
                    ...
                    <div class="response-section" style="min-width: 500px; max-width: 600px;">
                        ...
                        <p class="goal-reminder" style="background: ${
                          s.color
                        }15; border-left: 4px solid ${s.color};">
  ...
  timeline.push({
    ...
    stimulus: function () {
      ...
      return `<div class="completion-container">
                ...
                <p>You have finished describing all 10 trials to this listener as a <strong>${
                  s.role
                }</strong>.</p>
                ${
                  isLast
